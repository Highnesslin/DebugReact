...
function ChildReconciler(shouldTrackSideEffects) {
  ...
  function reconcileChildrenArray(
    ...
    for (; oldFiber !== null && newIdx < newChildren.length; newIdx++) {
      ...
      // previousNewFiber 新节点的前置节点，初值为null
    ...
    // 循环长度和新的children的长度相等：说明新数组 <= 老数组，表明老数组后面可能有剩余，所以要删除
    ...
    // 创建一个existingChildren代表所有剩余没有匹配掉的节点，然后新的数组根据key从这个 map 里面查找，如果有则复用，没有则新建
    // debugger
    const existingChildren = mapRemainingChildren(returnFiber, oldFiber);

    // Keep scanning and use the map to restore deleted items as moves.
    for (; newIdx < newChildren.length; newIdx++) {
      // 与updateSlot类似，区别在于对于新节点没有 key 的，会用 newIdx 作为索引从 existingChildren 这个 map 中获得对象，然后比较。
