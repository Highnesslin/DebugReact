...
export function updateContainer(
  ...
  debugger
  // 9. 从FiberRoot中取出current，这里的 container 就是刚才的 FiberRoot ， F10进入下一个端点
  ...
  debugger
  // 10. 创建一个更新对象，可以暂时不关注，继续下一个断点
  const update = createUpdate(expirationTime, suspenseConfig);
  // Caution: React DevTools currently depends on this property
  // being called "element".
  debugger
  // 11. 将element作为更新的参数 ， element就是 ReactDom.render接收的第一个参数，即入口组件 , 继续下一个节点
  ...
  debugger
  // 12. 由于 current 不包含更新队列，所以这里不会有实际执行
  enqueueUpdate(current, update);

  debugger
  // 13. 调度 Fiber 的更新行为， 有必要进去看下 ， 。
